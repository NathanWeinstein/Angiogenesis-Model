...
sortingHat <- function(state) {
	...
	if (state["NPR1"] == 1 && state["DLL4a"] == 1) {
		fate <- "Tip"
	}else if(state["JAGa"] == 1){ #state["HEY1"] == 1 | state["NICD"] == 1){ 
		fate <- "Stalk"
	}else if (state["AKT"] == 1){
		fate <- "Phalanx"
	}else{
...
sortAtractor <- function(atractorAnalisis){
	...
	fateFactor <- factor(Fate, c("Tip", "Stalk", "Phalanx", "EC"))
	...
	if(s["Tip"] == nr){
		...
		kind <- "EC"
	#}else if((s["EC"] + s["Tip"]) == nr){
	#	kind <- "ECTip"
	#}else if((s["EC"] + s["Stalk"]) == nr){
	#	kind <- "ECStalk"
	#}else if((s["EC"] + s["Phalanx"]) == nr){
	#	kind <- "ECPhalanx"
	#}else if((s["Tip"] + s["Phalanx"]) == nr){
	#	kind <- "PhalanxTip"
	#}else if((s["Stalk"] + s["Phalanx"]) == nr){
	#	kind <- "PhalanxStalk"
	#}else if((s["Stalk"] + s["Tip"]) == nr){
	#	kind <- "TipStalk"
	#}else if((s["EC"] + s["Tip"]) == nr){
	#	kind <- "ECTip"
	#}else if((s["EC"] + s["Phalanx"] + s["Tip"]) == nr){
	#	kind <- "ECPhalanxTip"
	}else{
		kind <- "Other"
	}
...
sortEnvironments <- function(attractors_by_environment){
	...
	for(i in 1:length(attractors_by_environment)){
		...
		}else if(length(aenv[aenv == "EC"]) == laenv){
			ec <- append(ec, nenv)
		}else{
			other <- append(other, nenv)
		}
	}
	sortedEnvironments <- list(Tip = tip, Stalk = stalk, Phalanx = phalanx, EC = ec, Other = other)
